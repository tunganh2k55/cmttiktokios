...
function findTextAndTap(targets, x1, y1, x2, y2, timeFind)
  timeFind = timeFind or 6
  local startTime = os.time()

  local list = {{}}
  if type(targets) == "table" then
    for _, v in ipairs(targets) do list[#list+1] = tostring(v):lower() end
  else
  ...
  while (os.time() - startTime) < timeFind do
    local opt = {{
      ...
      languages = {"en-US"},
      correct = true,
      timeout = 2,     -- mỗi lần OCR tối đa 2s
      debug = false,
    }}
...
class Proxy:
    ...
    def changeProxy(serverlocal, proxy:str):
        ip, port = proxy.split(":")
        message = f"""function tapAddCommentWait5s(image_path, rx, ry, rz, rt)
                local startTime = os.time()
                local timeout = 5
                local region = {{rx, ry, rz, rt}};
        ...
        function findTextAndTap(targets, x1, y1, x2, y2)
        local opt = {{
            method = OCR_METHOD.IOS_VISION,
            region = (x1 and y1 and x2 and y2) and {{x1, y1, x2, y2}} or nil,
            languages = {{'en-US'}},
    ...
    def wipeApp(serverlocal, appName:str):
        message = f"""function tapAddCommentWait5s(image_path, rx, ry, rz, rt)
                local startTime = os.time()
                local timeout = 5
                local region = {{rx, ry, rz, rt}};
    ...
    def regnvr(serverlocal, appName:str):
        message = f"""
        ...
        local password = random_password("vu", "tong", "phan")
        inputText(password)
        findTextAndTap("Next")
        ...
        local message = uid.."|".."devtestpass".."|"..text
...
localip = "172.20.10.3:8080"

# autoTouch.post_lua_payload(localip, message=Proxy.changeProxy(serverlocal, proxy="mb.xoayproxy.com:51566"), name_file="testreg.lua")
# autoTouch.get_playSource(localip, name_file="testreg.lua")

autoTouch.post_lua_payload(localip, message=Proxy.regnvr(serverlocal, appName="Facebook"), name_file="testreg.lua")
autoTouch.get_playSource(localip, name_file="testreg.lua")
